...
def check_vector_power_uniformity(vector, window_size=1024, max_db_delta=3):
    ...
    The original implementation used ``np.convolve`` which became extremely
    slow for large vectors.  This version uses a cumulative sum approach that
    scales linearly with ``len(vector)``.
    """
    ...
    # cumulative sum trick to compute moving average efficiently
    cumsum = np.cumsum(np.concatenate([[0.0], power]))
    window_power = cumsum[window_size:] - cumsum[:-window_size]
    rms = np.sqrt(window_power / window_size)
